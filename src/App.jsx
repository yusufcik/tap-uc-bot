...
/* MUHIM: BOT_TOKEN shu faylda saqlansa, ilovani ochgan har bir
   ...
   foydalanishi mumkin. Bu yechim shaxsiy/kichik loyihalar uchun
   yaroqli. Katta loyiha uchun bularni server (backend) orqali
   yashirin saqlash tavsiya etiladi. */
...
export default function TapUCApp() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await window.storage.get("tapuc:anon-id");
        setMyId(res.value);
        setMyName("Mehmon");
      } catch {
        const id = "guest_" + Math.random().toString(36).slice(2, 10);
        await window.storage.set("tapuc:anon-id", id).catch(() => {});
        setMyId(id);
        setMyName("Mehmon");
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get(TASKS_STORAGE_KEY, true);
        setTasks(JSON.parse(res.value));
      } catch {
        await window.storage.set(TASKS_STORAGE_KEY, JSON.stringify(DEFAULT_TASKS), true).catch(() => {});
  ...
  const saveTasks = async (next) => {
    setTasks(next);
    try {
      await window.storage.set(TASKS_STORAGE_KEY, JSON.stringify(next), true);
    } catch (e) {
      ...
      if (refCode && refCode !== myId) {
        (async () => {
          try {
            let list = [];
            try {
              const r = await window.storage.get(`ref:${refCode}`, true);
            ...
            if (!list.includes(myId)) {
              list.push(myId);
              await window.storage.set(`ref:${refCode}`, JSON.stringify(list), true);
            }
  ...
  const checkReferralEarnings = useCallback(async () => {
    if (!myId) return;
    try {
      const r = await window.storage.get(`ref:${myId}`, true);
